...
labels = [0, 0, 0, 1]  # result of data through an AND gate
plt.scatter([point[0] for point in data], [point[1] for point in data], c=labels, alpha=0.5)
plt.title('AND Gate')
plt.show()
# AND data set is linearly seperable... line with -1 slope and intercept > 1 would do it.

classifier = Perceptron(max_iter=40)
classifier.fit(data, labels)
print(classifier.score(data,labels))  # 1.0 ... no surprise


## OR Gate
data = [[0,0],[0,1],[1,0],[1,1]]
labels = [0, 1, 1, 1]  # result of data through an OR gate
plt.scatter([point[0] for point in data], [point[1] for point in data], c=labels, alpha=0.5)
plt.title('OR Gate')
plt.show()
# OR data set is linearly seperable... line with -1 slope and intercept < 1 would do it.
...
## XOR Gate
data = [[0,0],[0,1],[1,0],[1,1]]
labels = [0, 1, 1, 0]  # result of data through an XOR gate
plt.scatter([point[0] for point in data], [point[1] for point in data], c=labels, alpha=0.5)
plt.title('XOR Gate')
plt.show()
# XOR data set could not be linearly seperable, unless there is some mechanism to only classify objects ON a boundary

classifier = Perceptron(max_iter=40)
classifier.fit(data, labels)
print(classifier.score(data,labels))  #0.5 ... coin flip
...
classifier = Perceptron(max_iter=40)
classifier.fit(data, labels)
print(classifier.decision_function([[0,0],[1,1],[0.5,0.5]]))  # [-4.   1.  -1.5]
...
plt.title('Decision Boundary Heatmap for AND gate')
plt.show()
...
heatmap = plt.pcolormesh(x_values, y_values, distances_matrix)
plt.colorbar(heatmap)
plt.title('Decision Boundary Heatmap for OR gate')
plt.show()
...
heatmap = plt.pcolormesh(x_values, y_values, distances_matrix)
plt.colorbar(heatmap)
plt.title('Decision Boundary Heatmap for XOR gate')
plt.show()
